...
public:
  ...
  bool compValueStr(char* v, char* vToComp, AttrType tp);
  RC close();
...
public:
  ...
  void insertByRID(FileHandle &fileHandle, RID insertPlace, int& next_record_pos, int& recordSize, void* record);
  RID findFreePages_Slot(FileHandle &fileHandle, int needSpace, int& next_record_pos);
  void interpretRecord(const vector<Attribute> &recordDescriptor, const void* data, void* record, int* offset);
...
private:
  ...
  //int findFreePages(FileHandle &fileHandle, int needSpace);
  bool findSlotToInsert(FileHandle &fileHandle, int pgNum, void* page, int& next_record_pos, int& slotNum, int needSpace);

  // void getNullVector(int nullIndicatorLength, void* nullField, vector<bool> &isNull);
  int getPageFreeSpace(FileHandle &fileHandle, int pageNum);
  void organizePage(void* page, RID rid, void* update_record, int update_recordSize, int increase_space);
  bool isTomb(FileHandle &fileHandle, RID rid);
  RID getRidFromTombStone(FileHandle &fileHandle, RID rid);
  bool isDeleted(FileHandle &fileHandle, RID rid);
};
