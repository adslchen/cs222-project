...
int RBFTest_private_1(RecordBasedFileManager *rbfm) {
	...
	prepareRecordForTweetMessage(recordDescriptorForTweetMessage.size(),
			nullsIndicator, 101, 1, 0, "", 0, "", 123.4, 1013.45, record,
			&recordSize);
	...
	if (memcmp(record, returnedData, recordSize) != 0) {
		cout
				<< "***** [FAIL] Test Case Private 1 Failed on Reading Empty String Record! *****"
				<< endl << endl;
		free(record);
		free(returnedData);
		return -1;
	...
	// Insert a record
	prepareRecordForTweetMessage(recordDescriptorForTweetMessage.size(),
			nullsIndicator, 101, 1, 0, "", 0, "", 0, 1013.45, record,
			&recordSize);
	...
	if (memcmp(record, returnedData, recordSize) != 0) {
		cout
				<< "***** [FAIL] Test Case Private 1 Failed on Reading Nullable Record! *****"
				<< endl << endl;
		free(record);
		free(returnedData);
		return -1;
	...
	cout
			<< "***** RBF Test Case Private 1 Finished. The result will be examined! *****"
			<< endl << endl;
